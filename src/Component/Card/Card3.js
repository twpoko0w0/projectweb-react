...
function Card31({ items, searchTerm, nameTag, filterMemberCount, memberCount, count }) {


  function applyFilter() {
    const itemsEle = items.filter(x => {
...
export default function Card3({ items, searchTerm, filterMemberCount, memberCount }) {
  const [show, setShow] = useState(false);
  const [count, setCount] = useState(6)
  const props = { items, searchTerm, filterMemberCount, memberCount, count }
      ...
      <Container fluid="lg" style={{ maxWidth: "1140px", padding: "0" }}>
        <div className="grd" >
          ...
          <div
            className="butTom"
          >
            {count < items.length ? <Button
              className="show-btn"
              onClick={() => setCount((currCount) => currCount + 3)}
            >
              ดูเพิ่มเติม
            </Button> : null}
          </div>
